...
import banglaToBanglish from "../../utils/banglaToBanglish";
...
import { addImage, clearImages } from "../../features/productSlice/productSlice";
...
export default function AddProduct({ showModal, handleModal }) {
    const [input, setInput] = useState(initialInput);
    const { images } = useSelector(productSelector);
    const { user } = useSelector(accountSelector);
    const [imageUploadDoneOnce, setImageUploadDoneOnce] = useState(true);

    const [addProductServer, { isLoading: isAddVideoLoading }] =
        useAddProductMutation();
    const { data: categories, isSuccess: isSuccessCategory } =
        useGetCategoriesQuery();
    const dispatch = useDispatch();
    const [
        productImageUploadServer,
        {
            isSuccess: isProductImageUploadServerSuccess,
            isLoading: isProductImageUploadServerLoading,
        },
    ...
    const handleSubmit = (e) => {
        ...
        addProductServer(data)
            .unwrap()
            .then((response) => {
                images?.forEach((image) => {
                    productImageAddServer({ image, product: response.id })
                        .unwrap()
                        .then(() => {
    ...
    return (
        <>
                        ...
                        <div className="p-4 md:p-5">
                            <form
                                ...
                                <button
                                    ...
                                    disabled={
                                        isAddVideoLoading ||
                                        !isProductImageUploadServerSuccess
                                    }
            ...
            {isProductImageUploadServerLoading && <CircleLoading />}
        </>
    );
}
